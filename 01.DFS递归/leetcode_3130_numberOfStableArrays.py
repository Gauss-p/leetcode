...
class Solution:
    def numberOfStableArrays(self, zero: int, one: int, limit: int) -> int:
        ...
        def dfs(i, j, k):
            ...
            # 注意减去不合法情况
            if k == 0:
                return (dfs(i-1, j, 0)+dfs(i-1, j, 1)-(0 if i<=limit else dfs(i-limit-1, j, 1)))%modNum
            else:
                return (dfs(i, j-1, 0)+dfs(i, j-1, 1)-(0 if j<=limit else dfs(i, j-limit-1, 0)))%modNum
        
        res = (dfs(zero, one, 0) + dfs(zero, one, 1))%modNum
        dfs.cache_clear()
        return res
