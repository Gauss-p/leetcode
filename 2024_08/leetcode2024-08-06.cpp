...
public:
    int numberOfStableArrays(int zero, int one, int limit) {
        ...
        // 继续分析递推式，f[i][j][0]的值就是用i个0，j个1组成的合法数组最后再加上0能形成的合法数组个数，那么反推前一位，也就是第i+j-1位上的数字有两种可能，要么是0，要么是1，这两种情况的和就是f[i-1][j][0]+f[i-1][j][1]其中i减了1是因为最后一位要再多用一个0。但是又有一种情况，就是如果f[i-1][j][0]这种情况中有部分数字最后有连续limit个0，那如果再加一个0，就不合法了，所以说，还要再减去这种情况，而这种情况的总数就是f[i-limit-1][j][1]，之所以最后填1，是因为最后如果有连续limit个0，而它又合法，那么倒数limit+1位上的数字必然是1，由上面的分析可以得出，如果i>limit，那么f[i][j][0] = f[i-1][j][0]+f[i-1][j][1]-f[i-limit-1][j][1]，否则f[i][j][0]=f[i-1][j][0]+f[i-1][j][1]
